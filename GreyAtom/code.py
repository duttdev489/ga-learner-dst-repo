...
def read_file(path):
    ...
    sentence = ''.join ( file.readlines(0))
    #Closing of the file
    file.close()
...
def compare_msg(message_d,message_e):
    ...
    for i in message_d.split(' '):
        a_list.append(i)
    
    #Splitting the message into a list
    for i in message_e.split(' '):
        b_list.append(i)
    #print(a_list)
    #print(b_list)
...
#secret_msg = ' '.join(str(message_parts))
secret_msg=''
for msg in message_parts:
    secret_msg += str(msg) + ' '
secret_msg =secret_msg.strip()
...
def write_file(secret_msg,path):
    ...
    secret_message = open(path,mode='w')
